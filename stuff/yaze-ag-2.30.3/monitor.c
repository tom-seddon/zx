...
showdisk(int disk, int verbose)
{
	...
	if (dp->flags & MNT_UNIXDIR)
	    ...
	    switch (version) {
		case 0: printf("yaze 1.10/1.06 (version 0)"); break;
		case 1: printf("yaze-ag 2.xx (version 1)");   break;
...
mountdir(struct mnt *dp, const char *filename, unsigned int labelb)
{
    ...
    cp = cpmdir; *cp = 0xE5; /* E5H --> erased directory entry */
    cp += 32; *cp = 0xE5;
    cp += 32; *cp = 0xE5;
    cp += 32; *cp = 0x21; /* 21H -> date stamps directory entry */
	...
	for (i = 0; i < ndirents; i++) {
		...
		*cp = 0x21;
		if (++direno2 < N_ENTRIES) {
		    /* setup next sektor */
		    cp = cpmdir + (direno2)*32; *cp = 0xE5;
		    cp += 32; *cp = 0xE5;
		    cp += 32; *cp = 0xE5;
		    cp += 32; *cp = 0x21; /* date stamps */
		    datestamps = cp + 1;
		}
	    }
	}
