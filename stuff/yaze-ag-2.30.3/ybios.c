/* Basic i/o system module (yaze-bios).
   ...
   Modifications for CP/M 3.1 Copyright (C) 2000/2009 by Andreas Gerlich (agl)
...
new_iobyte(int io)
{
    ...
    old_iobyte = io;
    chn[CHNaux] = AUX;	/* nessasary for CP/M 3 */
}
...
seldsk(int disk)
{
    ...
    /*	Superdos (CP/M 2.2 derivate) uses like CP/M 3.1 (or ZPM3) also the
	...
	make a reread of a disk which is connected to a unix directory */
...
/*	date function:
	...
	You find on drive M: (Turbo-Modula-2) the source in Modula-2.
	(Look to DAYS.MOD/DEF and BIRTHDAY.MOD)

	I extrakt the formula from a TI-58 pocket calculator !
...
{   int d, len = (GetBYTE(0x80) & 0x7f); /* get lengt of filepath at 0080H */
    for (d = 0; d < len - 1; d++)
	cpmCommandLine[d] = GetBYTE(0x82 + d);
...
*/
	...
	/* Sethreg(AF, serin(CHNconin) ); */
	ch = serin(CHNconin);

/* following keyreplacements are only for windows */
    ...
    case 11:			/* set sector */
	 ...
	 if (!ucsdmode) {
	   ...
	   sector-- ;   /* decrement one because the first sector for the
			   UCSD P-System is sektor one and not zero, like the
	...
	break;			/* This given back the BIOSKRNL.Z80 directly.
				   ...
				   and a RET. Have a look to Page 53 of the
...
***/
	...
	/* structur for load MMUtable in the z80-mem (HL is the pointer):
	   ...
	   return-codes (reg A & HL):
		A = 0xFE: PagePointer is wrong (out of Memory). HL points
			  to the wrong PP.
		A = 0xFF: MMUtable (first Byte) numbers an MMUtable which does
			  not exist.
	*/
	loadMMU();
	break;
	...
	fprintf(stderr,"\r\n\nYAZE: Invalid bios function: %d (0x%X)\r\n\n",
								     func,func);
	fprintf(stderr,"YAZE: You try to use functions for the MMU or CP/M 3.1.\r\n");
